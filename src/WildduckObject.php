...
class WildduckObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToWildduckObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Wildduck Notice: Undefined property of {$class} instance: {$k}. "
        ...
        $class = \get_class($this);
        Wildduck::getLogger()->error("Wildduck Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof WildduckObject) {
    ...
    public function updateAttributes($values, $opts = null, $dirty = true)
    {
        foreach ($values as $k => $v) {
            ...
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = WildduckObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToWildduckObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            if ($force || $unsaved || $v instanceof WildduckObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToWildduckObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof WildduckObject) {
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof WildduckObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof WildduckObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
